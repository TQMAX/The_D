...
//            }
            ...
            var TheChapterID = chapterID
            if chapterID == -1 {
                TheChapterID = readModel.lastChapterID
            }

            // 获取当前需要阅读的章节
            let url = "https://shuapi.jiaston.com/book/" + bookID! + "/" + TheChapterID!.stringValue + ".html"
            ...
            Alamofire.request(url, method: .post, parameters: nil ).responseJSON { (response) in
                ...
                if let baseModel = JSONDeserializer<BasicTypes>.deserializeFrom(dict: dic){
                    ...
                    // 解析章节数据
                    let tempChapterModel = DZMReadChapterModel()
                    
                    tempChapterModel.bookID = baseModel.data?.id
                    
                    tempChapterModel.id =  baseModel.data?.cid
                    
                    tempChapterModel.previousChapterID =  baseModel.data?.pid
                    
                    tempChapterModel.nextChapterID = baseModel.data?.nid
                    
                    tempChapterModel.name = baseModel.data?.cname
                    
                    // 章节类容需要进行排版一篇
                    tempChapterModel.content = DZMReadParser.contentTypesetting(content: data ?? "")
                    
                    // 保存
                    tempChapterModel.save()
                    
                    // 修改阅读记录
                    recordModel.modify(chapterID: TheChapterID, toPage:0 , isSave: false)
                    
                    // 更新阅读记录
                    self.updateReadRecord(recordModel: recordModel)
                    
                    // 展示阅读记录
                    self.creatPageController(displayController: self.GetReadViewController(recordModel: recordModel))
...
//                }
                // 获取当前需要阅读的章节
                let url = "https://shuapi.jiaston.com/book/" + bookID! + "/" + chapterID!.stringValue + ".html"
                ...
                Alamofire.request(url, method: .post, parameters: nil ).responseJSON { (response) in
                    ...
                    if let baseModel = JSONDeserializer<BasicTypes>.deserializeFrom(dict: dic){
                        ...
                        // 修改阅读记录
                        recordModel.modify(chapterID: tempChapterModel.nextChapterID, toPage:0 , isSave: false)
