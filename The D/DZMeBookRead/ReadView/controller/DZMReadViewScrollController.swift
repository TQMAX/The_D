...
//        }
    ...
    private func preloadingPrevious(_ chapterModel:DZMReadChapterModel!) {
        ...
        DispatchQueue.global().async { [weak self] () in
            ...
            // 存在 || 不存在(但是为本地阅读)
            if isExist || self?.vc.readModel.bookSourceType == .local || chapterID == -1{
...
//                }
                 // 如果不存在则需要加载网络数据
                    ...
                    // 获取当前需要阅读的章节
                    let url = "https://shuapi.jiaston.com/book/" + chapterModel.bookID + "/" + chapterModel.previousChapterID.stringValue + ".html"
...
//                                // 如果存在则修改阅读记录
//                                readModel.recordModel.modify(chapterID: chapterModel.nextChapterID, location: 0)
//                            }
//
...
//                }
                ...
                if chapterID == -1 {
                    TheChapterID = chapterModel.lastChapterID
                }
                
                // 获取当前需要阅读的章节
                let url = "https://shuapi.jiaston.com/book/" + chapterModel.bookID + "/" + TheChapterID!.stringValue + ".html"
